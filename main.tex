...
\author{
\setlength{\tabcolsep}{0pt}% kontrol jarak kolom manual
\begin{tabular}{@{}c@{\hspace{0.045\textwidth}}c@{\hspace{0.045\textwidth}}c@{}}

% ================= AUTHOR 1 =================
\begin{minipage}[t]{0.28\textwidth}
\centering
\vspace{0pt}
{Christopher Ricky Soehartono}\\[2pt]
...
\centering
\vspace{0pt}
{Ir. Andry Chowanda, S.Kom, MM, Ph.D.,}\\[-1pt]
{MBCS, CCP, CME, IPM, SMIEEE}\\[2pt]
\textit{Computer Science Department}\\
\textit{School of Computer Science}\\
\textit{Bina Nusantara University}\\
Jakarta, Indonesia 11480\\
...
Wi-Fi sensing uses channel state information (CSI) from commodity Wi-Fi devices to
recognise human activities without cameras or additional sensing hardware
\cite{Chen2022Survey,Radwan2025Survey}. Its main challenge is poor generalisation across
environments: CSI captures both human motion and the surrounding propagation conditions,
so the same activity can produce different signals when the room, device placement, or
orientation changes \cite{Chen2022Survey,Wang2025GeneralizabilitySurvey,
Zhang2026BeyondLabels}.

Existing solutions mainly use \emph{domain adaptation} (DA), \emph{domain
generalisation} (DG), few-shot learning, or self-supervised learning. DA aligns source and
target distributions but requires target-domain data
\cite{Yang2021BookChapter,Chen2022Fidora,Mao2024WiCro,Prasad2024DANN}. DG and
self-supervised approaches instead learn representations that transfer across domains
\cite{Liu2024WiSR,Liu2024WiSGP,Wang2022AirFi,Xu2023ARCFi,Yang2022AutoFi,
Barahimi2024CPC,Lyons2024WiFiAct}, while few-shot methods adapt using a small number of
target examples \cite{Yin2022FewSense,Zhao2024CrossFi,Yang2023FewCS}. However, making a
representation fully invariant to the environment may discard useful contextual
information. Disentanglement provides another option by separating activity information
from environment-specific factors \cite{Kang2021ContextAware}.

Retrieval-augmented generation (RAG) offers a way to preserve and reuse such contextual
information. Wang et al.\ \cite{Wang2025GeneralizabilitySurvey} identify external
retrieval as a possible direction for Wi-Fi sensing, while RAG has already been explored
in language models \cite{lewis2020rag}, wireless systems
\cite{Nazar2024Enwar,Nazar2026Enwar2,wi_chat}, and spatial reasoning
\cite{spatial_rag}. However, existing work does not use retrieval of spatial or layout
information as the adaptation mechanism for cross-domain CSI sensing.

We therefore propose \method{}, a framework that combines self-supervised
disentanglement with spatial retrieval. \method{} separates CSI into a \emph{content}
code representing the activity and a \emph{domain} code representing environmental
conditions. At inference time, the domain code retrieves relevant entries from a
geometry-tagged memory, and the retrieved context is used to condition the classifier.
A new environment can therefore be added through memory insertion without labelled
target data or gradient-based retraining.

\subsection*{Contributions}
\begin{itemize}
\item We formulate cross-domain CSI sensing as \emph{retrieval-conditioned perception},
preserving domain information as a retrieval key instead of discarding it
(Section~\ref{sec:method}).
\item We introduce a self-supervised disentanglement objective that separates activity
and domain factors using physics-informed augmentations and an independence penalty
(Section~\ref{sec:method}).
\item We define an evaluation protocol covering DA, DG, and few-shot baselines, together
with a memory-free ablation to isolate the contribution of retrieval
...
\midrule
$x$ & CSI input window. \\
$N_s, N_t$ & Number of subcarriers and time samples in a CSI window. \\
$y, C$ & Activity label and number of activity classes. \\
$d$ & Domain descriptor available at inference time. \\
$E_\theta$ & Encoder that maps a CSI window to content and domain codes. \\
$D_\psi$ & Decoder used for CSI reconstruction. \\
$g_\phi$ & Activity-classification head. \\
$\zc, \zd$ & Content code and domain/environment code, respectively. \\
$\mathcal{M}$ & Spatially indexed retrieval memory. \\
$\rho_m, e_m$ & Geometric descriptor and conditioning embedding of memory entry $m$. \\
$\phi_{\mathrm{geom}}(\cdot)$ & Function that extracts a geometric descriptor from CSI. \\
$K, S$ & Number of retrieved entries and the resulting top-$K$ set. \\
$s(\cdot,\cdot)$ & Similarity function used for memory retrieval. \\
$w_m$ & Softmax retrieval weight assigned to memory entry $m$. \\
$\tau$ & Softmax temperature for retrieval weighting. \\
$\delta$ & Similarity threshold for the out-of-memory decision. \\
$\mathcal{L}_{\mathrm{rec}}$ & Reconstruction loss. \\
$\mathcal{L}_{\mathrm{cls}}$ & Activity-classification loss. \\
$\mathcal{L}_{\mathrm{ind}}$ & Independence/disentanglement loss. \\
$\mathcal{L}_{\mathrm{con}}$ & Content-consistency loss across augmented views. \\
$\lambda_{1..3}$ & Weights of the classification, independence, and consistency losses. \\
\bottomrule
\end{tabular}
\end{table}
